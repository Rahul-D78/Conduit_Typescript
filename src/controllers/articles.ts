...
export async function createArticle(data: ArticleData, email: string): Promise<Article> {
    ...
    // const exist = articleRepo.findOne((data.title))
...
export async function getArticleBySlug(slug: string) {
    try {
        const repo = getRepository(Article)
        const article = repo.findOne(slug)
...
export async function updateArticle(data: ArticleData, slug: string, email: string): Promise<Article> {
    ...
    try {
       ...
       if(user) sanitization(user as any)
...
export async function deleteArticle(slug: string, email: string) {
    ...
    try {
        ...
        const user = repoU.findOne(email)
